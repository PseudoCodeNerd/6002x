...
# Problem 3: StandardRobot Class 10.0/10.0 points (graded)
...
# Ordinarily we would consider putting all the robot's methods in a single class. However, later in this problem set
# we'll consider robots with alternate movement strategies, to be implemented as different classes with the same
# interface. These classes will have a different implementation of updatePositionAndClean but are for the most part
# the same as the original robots. Therefore, we'd like to use inheritance to reduce the amount of duplicated code.
...
# line under your implementation of StandardRobot. Make sure that as your robot moves around the room, the tiles it
# traverses switch colors from gray to white. It should take about a minute for it to clean all the tiles.
...
class StandardRobot(Robot):
    ...
    def updatePositionAndClean(self):
        ...
        been cleaned.
        """
        new_pos = self.curr_posn.getNewPosition(self.direcn, self.speed)
